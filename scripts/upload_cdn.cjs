...
'use strict'
/**
 * CDN 资源上传脚本 — 无尽纹章（GameKey = wujin_wenzhang）
 *
 * 通过腾讯云 COS REST API 增量上传到 CloudBase 存储桶。
 * 上传目标：cdnConfig.cloudbaseBucket，路径前缀 wujin_wenzhang/assets_cdn
 *
 * 用法:
...
const { loadUploadEnv, PROJECT_ROOT } = require('./loadEnv.cjs')
const cdnCfg = require(path.join(PROJECT_ROOT, 'scripts', 'cdnConfig.cjs'))
...
async function main() {
  console.log('=== 无尽纹章 CDN 资源上传（腾讯云 COS / CloudBase）===')

# 炎弹弹体（ember_orb）—— 黑底 additive 火球 6 帧（2x3 网格，循环）

- 用途：法师普攻与技能「炎弹」的飞行弹体，`vfxCatalog` 的 `travel.glowSet='ember_orb'`。
- 运行时 `vfxProjectile` 会 **loop** 这段动画，所以 6 帧必须是可无缝循环的脉动，不是出现→消失。
- 色相家族：**赤焰**（炎系法师）。白热核心 → `#FF4A12` → `#B01400`。
  比剑士金橙更红，形态是火球不是斩击。禁止青、紫、金黄。
- 形态：一颗朝右的泪滴火球。不是贯穿光束（`pierce`），也不是双向尖刺（`arrow_hit`）。
...
A 2x3 sprite sheet (2 rows, 3 columns) showing 6 frames of a FIREBALL — a compact glowing orb of
real fire flying to the RIGHT, seen from DIRECTLY ABOVE (top-down). This is a LOOPING projectile:
...
SHAPE: a teardrop / comet of fire pointing RIGHT. Fat round head on the right, tapering tail on
the left. The head is a bright circular core about 28% of the cell width. The tail is short —
about as long as the head, never more than 55% of the cell width from tip to tail-end. Two or
three chunky flame tongues on the tail. This is a BALL of fire with a short tail, not a lance,
not a horizontal beam, not two opposing spikes.

COLOR: white-hot / pale yellow at the core of the head, blazing orange-red #FF4A12 in the mid
flame, deep crimson #B01400 at the tail tip and outer glow. Warm fire colours only. No purple,
no violet, no cyan, no blue, no gold-yellow slash colour, no green.
...
brightness oscillate, they do not grow then die:
  ...
  Frame 6 (row2 col3): Head about 22%, dimmest of the loop, tail a short stub. Next would be
                       frame 1 again. Still clearly a fireball — do not fade to black.
...
4. The fireball stays HORIZONTAL, pointing RIGHT, vertically CENTERED in its cell in all 6 frames.
...
7. NO arrow, NO arrowhead, NO shaft, NO character, NO hat, NO staff, NO ground, NO terrain, NO
   grass, NO shadow. Only the abstract glowing fireball on black.

 ...
 *
 * 玩家状态假设 —— 刚通关第一章、没有刷关：
 *   第一章首通的 meta 收入是 3 个战斗节点 ×2 + Boss×5 + 通关 10 = 21 灵魂，
 *   而升级价格是 `3 + 当前等级 × 2`（1→2 要 5，2→3 要 7）。三个角色全升到 2 级
 *   花 15，剩下 6 点不够再升一级——所以**统一按 2 级**算。
 *   （第一章从七关缩到四关之后这笔收入从 27 降到 21，但 2 级这条结论没变。）
 *   精华（`bonusAtkEach`）在章内从 2 累积到 4，沿用第一章那条「每两场战斗 +1」的斜率。
 *
 ...
 * 这一章的关卡组成变过一次（原第一章的「骑兵突袭」挪了进来、「涸河林隘」挪去第四章），
 * 硬写的下标会静默错位到别的章节的关卡上，而胜率测试照样能跑出一个「看起来合理」的数字。
 */
describe('第二章难度曲线回归', () => {
  const stages = battleStageIndices(DUNGEON);

  /** 章内精华累积：关 5-6 两点，关 7-8 三点，Boss 四点 */
  const BONUS_BY_STAGE = [2, 2, 3, 3, 4];
  const LEVEL = 2;
   ...
   * 推进关目标 ≥75%。
   *
   * 比第一章的引导关（实测 95% 以上）低一档是有意的：那是教操作的四关，
   * 这一章的推进关要开始要求读地形。但**不能低到卡关**——难度该住在精英关和 Boss 关，
   * 推进关如果也要重开，玩家会在同一段路上反复走，而那段路他已经会了。
   */
  it('关 5-7 推进关：要求读地形，但不卡关', () => {
    const labels = ['关5 藤蔓小径', '关6 骑兵突袭', '关7 松脂林道'];
  ...
  /** 精英关目标 55%-90%（与第一章精英关同档） */
  it('关 8 精英战：明显是个台阶，但不是第二个 Boss', () => {
    const r = simulateStage(cfg(3), N);
    report('关8 精英', r, '55%~90%');
    expect(r.winRate, `关8 胜率 ${(r.winRate * 100).toFixed(1)}%`).toBeGreaterThanOrEqual(0.55);
    expect(r.winRate, `关8 胜率 ${(r.winRate * 100).toFixed(1)}%`).toBeLessThanOrEqual(0.9);
  }, 60_000);

  /**
   * Boss 关：裸打惩罚（实测 ~34%）、备药后稳过（实测 ~92%）。
   *
   * 这个落差和第一章同形（那边是 27% / 74%），是「Boss 战要备药」这条既有设计的结果，
   * 不是这一关的问题。裸打留在三成而不是个位数，是因为第二章的
   ...
   */
  it('关 9 Boss：裸打惩罚、备药后可过，且会放燎原咒火', () => {
    const naked = simulateStage(cfg(4), N);
    report('关9 Boss 裸打', naked, '15%~45%');
    ...
    const prepared = simulateStage(cfg(4, { healPotions: 2 }), N);
    report('关9 Boss 带 2 药', prepared, '>=85%');
    expect(
      prepared.winRate,

 ...
 * 半透明黑能跟着背景走，换背景不用回来改。
 */
export type CurrencyPill = PIXI.Container & {
  setText: (text: string) => void;
};

export function createCurrencyPill(iconKey: string, text: string): CurrencyPill {
  const c = new PIXI.Container() as CurrencyPill;
  const ICON = 20;
  const PAD = 8;

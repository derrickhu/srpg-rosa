...
/**
 * 贴在图标背后的一小圈漏光。锚在父节点原点。
 *
 * 药剂掉落本身不带光；觉醒徽也只是图后两团淡金圆。纹章掉落跟那套走：
 * 光垫在图底下，只从边缘漏一圈，不画射线、不铺格子。
 */
export function attachCircleGlow(
  ...
  halo.beginFill(color, 0.2);
  halo.drawCircle(0, 0, radius * 1.06);
  halo.endFill();
  layer.addChild(halo);

  const core = new PIXI.Graphics();
  core.blendMode = PIXI.BLEND_MODES.ADD;
  core.beginFill(color, 0.22);
  core.drawCircle(0, 0, radius * 0.7);
  core.endFill();
  layer.addChild(core);
  ...
  const step = (): void => {
    ...
    layer.alpha = 0.62 + Math.sin(ticker.lastTime / 420) * 0.1;
  };
  if (ticker.started) ticker.add(step);
  return {
    setActive(on: boolean) {

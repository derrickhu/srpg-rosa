...
PIC_SPECS = [
    ...
    ('11_challenge.jpg', '图11 副本界面 - 无尽试炼入口',
     '展示副本页、无尽试炼说明与挑战入口。'),
    ('12_endless.jpg', '图12 无尽试炼战斗',
     '展示无尽试炼同图连战、当前波次与拾取药剂。'),
...
def write_document(pdf):
    ...
    pdf.write_body(
        ...
        '通过冒险章节推进主线: 进入节点后先布阵再自动/半自动战斗, 战后从三选一战利品中强化本局队伍, '
    ...
    pdf.write_body(
        ...
        '在己方部署行内放置单位, 并可消耗本局地形券改变格子地形。确认后开战, 进入战斗回放。'
    )
    ...
    pdf.write_body(
        ...
        '盾卫职业带嘲讽, 弓手远程攻击, 骑兵移动力更高。玩家可使用本局药剂, '
        '也可在自动战斗与手动回合之间切换。敌方由战斗引擎 AI 驱动。'
        '我方全灭判定失败; 清空敌军判定胜利。'
    )
    ...
    pdf.write_body(
        '无尽试炼中击杀掉落的药剂需要单位走过去待机拾取。每清一波当场入账魂晶, '
    ...
    pdf.write_body(
        ...
        '输出回放指令。battle/ai.ts 为敌军选择目标与走位。skills.ts 与 skillDamage 计算技能形状与伤害。'
        'BattlePlaybackView 消费指令播放动画、飘字与特效, 并处理手动回合输入。'
    )
    show_pic(pdf, '08_battle.jpg')
    pdf.write_table_auto(
        ['子模块', '功能说明'],
    ...
    pdf.write_body(
        ...
        'terrainSpec 定义地形效果; 地形券按类型计数, 放置后从本局库存扣除。'
        '开战时 undoDeployForRetry 支持失败后重新布阵。'
    )
    show_pic(pdf, '07_deploy.jpg')
